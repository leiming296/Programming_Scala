...
object DispatchersSpec {
  ...
  class ThreadNameEcho extends Actor {
    def receive = {
      case _ â‡’ sender() ! Thread.currentThread.getName
    }
  }
...
class DispatchersSpec extends AkkaSpec(DispatchersSpec.config) with ImplicitSender {
  ...
  def instance(dispatcher: MessageDispatcher): (MessageDispatcher) â‡’ Boolean = _ == dispatcher
  def ofType[T <: MessageDispatcher: ClassTag]: (MessageDispatcher) â‡’ Boolean = _.getClass == implicitly[ClassTag[T]].runtimeClass

  def typesAndValidators: Map[String, (MessageDispatcher) â‡’ Boolean] = Map(
  ...
  lazy val allDispatchers: Map[String, MessageDispatcher] = {
    validTypes.map(t â‡’ (t, from(ConfigFactory.parseMap(Map(tipe -> t, id -> t).asJava).
  ...
  "Dispatchers" must {
    ...
    "get the correct types of dispatchers" in {
      ...
      assert(typesAndValidators.forall(tuple â‡’ tuple._2(allDispatchers(tuple._1))))
    }
